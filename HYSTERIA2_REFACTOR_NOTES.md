...
## Совместимость
Obfs из метаданных host/inbound теперь приоритетен во ВСЕХ форматах, включая
ссылки. Старые пользовательские obfs-поля читаются только если оба канонических
ключа отсутствуют. В штатном генераторе эти ключи присутствуют всегда.
Пользовательский obfs больше не перекрывает настройки хоста только в ссылках.
