...
class GlamourPinkThemeTests(unittest.TestCase):
 ...
 def test_shared_panels_use_glass_surface(self):
  panel=(ROOT/"app/dashboard/src/components/Panel.tsx").read_text();self.assertIn('backdropFilter="blur(18px) saturate(130%)"',panel);self.assertIn('borderRadius="16px"',panel)
 def test_navigation_uses_soft_pink_active_state(self):
  sidebar=(ROOT/"app/dashboard/src/components/Sidebar.tsx").read_text();self.assertIn("var(--theme-active-gradient)",sidebar);self.assertIn("rgba(246,83,173,.20)",(ROOT/"app/dashboard/src/theme/palettes.scss").read_text())
 def test_technical_surfaces_are_preserved(self):
  self.assertIn("AnsiLogViewer",(ROOT/"app/dashboard/src/components/AnsiLogViewer.tsx").read_text());editor=(ROOT/"app/dashboard/src/components/JsonEditor/index.tsx").read_text();self.assertIn("<Editor",editor);self.assertIn('theme={`marzban-${theme}`}',editor)
