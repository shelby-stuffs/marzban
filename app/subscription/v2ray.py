...
class V2rayShareLink(str):
    ...
    def add(self, remark: str, address: str, inbound: dict, settings: dict):
        net = inbound["network"]
        # RAW is the v26 rename of TCP and is wire-identical to it. Emit the
        # universally understood "tcp" label in share links so that clients
        # with older cores keep importing these links correctly.
        if net == "raw":
            net = "tcp"
...
class V2rayJsonConfig(str):
    ...
    def splithttp_config(self, path: str = "", host: str = "", random_user_agent: bool = False,
        ...
        config = copy.deepcopy(
            self.settings.get(
                "xhttpSettings",
                self.settings.get("splithttpSettings", {}),
            )
        )
        ...
        if host:
            config["host"] = host
        if random_user_agent:
            config.setdefault("headers", {})["User-Agent"] = choice(
                self.user_agent_list
            )
        extra = {}
        if sc_max_each_post_bytes is not None:
    ...
    def stream_setting_config(network=None, security=None,
        ...
        method = method_map.get(network, network or "raw")
        settings_key = {
            "raw": "rawSettings",
        ...
        }.get(method, f"{method}Settings")

        streamSettings = {"method": method}
        ...
        if network_setting:
            streamSettings[settings_key] = network_setting
