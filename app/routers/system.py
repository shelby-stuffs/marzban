...
def get_system_stats(
    ...
    return SystemStats(
        version=__version__, mem_total=mem.total, mem_used=mem.used,
        cpu_cores=cpu.cores, cpu_usage=cpu.percent, total_user=total_user,
        online_users=online_users, users_active=users_active,
        users_disabled=users_disabled, users_expired=users_expired,
        users_limited=users_limited, users_on_hold=users_on_hold,
        incoming_bandwidth=system.uplink, outgoing_bandwidth=system.downlink,
...
@router.get(
    "/hosts", response_model=Dict[str, List[XHTTPProxyHost]], responses={403: responses._403}
)
def get_hosts(
...
def modify_hosts(
    modified_hosts: Dict[str, List[XHTTPProxyHost]],
