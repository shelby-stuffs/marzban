...
export const Hysteria2SettingsPage = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const response = await fetch<SettingsResponse>("/hysteria2");
      setMeta(response);
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      const response = await fetch<{ settings: HysteriaSettings; source: string }>("/hysteria2/generate", { method: "POST" });
      setForm(response.settings);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const response = await fetch<SettingsResponse>("/hysteria2", { method: "PUT", body: form });
      setMeta(response);
      setForm(response.settings);
  ...
  const showPreview = async () => {
    setPreviewing(true);
    try {
      const response = await fetch<{ config: Record<string, unknown>; user_count: number }>("/hysteria2/runtime-config");
  ...
  if (loading || !form) return <Box><Header title={t("hysteria.title")} /><Spinner mt="4" /></Box>;

  return (
    <VStack align="stretch" spacing="4" w="full">
      <Header title={t("hysteria.title")} />
...
export default Hysteria2SettingsPage;
