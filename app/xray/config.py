    ...
    def _resolve_inbounds(self):
        for inbound in self['inbounds']:
            ...
            if stream := inbound.get('streamSettings'):
                method = stream.get('method') or stream.get('network', 'raw')
                method_aliases = {
                    'tcp': 'raw',
                    'ws': 'websocket',
                    'kcp': 'mkcp',
                ...
                net_settings = next(
                    (stream[key] for key in settings_keys if isinstance(stream.get(key), dict)),
                    {},
                )
                net = internal_network
                ...
                if security == 'tls':
                    ...
                    settings['pbk'] = (
                        tls_settings.get('password')
                        or tls_settings.get('publicKey')
                    )
                    if not settings['pbk']:
                        ...
                        try:
                            ...
                            if not x25519:
                                raise ValueError(
                                    f"Could not parse x25519 output from xray core for {inbound['tag']}"
                                )
                        ...
                        if not settings.get('pbk'):
                            raise ValueError(
                                f"You need password (publicKey) in realitySettings of {inbound['tag']}"
                            )
